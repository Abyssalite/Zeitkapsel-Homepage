...
### Starting Containers
To ensure proper operation, use following command to start all service in correct order:
```bash
task all:start
```
...
```
  ...
  SERVICE_LIST:    
    ...
    - service-proxy
```
2. Restart all service using:
```bash
task all:restart
```
3. Restart specific list using:
```bash
task restart P="managerment"

task restart P="service"
```
4. Restart specific service using:
```bash
task restart P="webapp"

task restart P="webapp service-proxy"
```
...
### Other service
#### Cockpit
- Does not provided an intergration with keycloak(not require for internal managerment tool)
- require secure web sockets http 1.1 to function

#### Elabftw
- only function with mysql:8.4
- Does not support OIDC 
- only provide SAML authentification
- SAML setup is complicated and lacking documentation
- The only detail guide (https://github.com/elabftw/elabftw/discussions/5329) failed to what Certificate should be used and used manual setting instead of Add IDP(s) from URL
- compare to other solution like openBIS, Elabftw still feel more intuitive
- OIDC can still be use when using an other keycloak OIDC as a identity provider for SAML

#### Fuseki
- Fuseki volume cannot be mount to host system due to rootless permission restrain
- Can only use podman volume for persistance data
- Does not support OIDC natively
- Fuseki can still be authenticate with OIDC using oauth2-proxy and trust forwarding header
  
#### Jupyter/JupyterHub
- jupyterhub docker image does not included require package for oauthenticator support, dockerspawner and postgres => rebuild image
- Similarly jupyterlab image does not provide require plugin for research like git, latex, task, language server. Rebuild jupyterlab image with these too bollune the image size
- => rebuild from jupyter foundation produce a manageable image size with full funtionalities 
- Jupyter cross origin error behind reverse proxy can be solve with https://github.com/jupyterhub/jupyterhub/issues/1087

#### prometheus/grafana
- prometheus podman-exporter require userns=keep-id:uid=65534 to be set to function properly. But this setting in not applied in podman-compose podman-exporter has to run by podman run -d --name podman-exporter
- promql though share some resemblance to sql and sparql, still persent some challenge
- Grafana dashboaed visualization is powerfull but have a steep with promql
- alerting Matrix webhook Contact point Custom Payload template can only be Overrides with limited option definded in https://grafana.com/docs/grafana/latest/alerting/configure-notifications/template-notifications/reference/


#### matrix/hookshot
- by far the most complicated to setup
- configuation(homeserver.yaml) can nolonger be passthrough by podman 
- require a podman run synapse generate to generate config file
- the config file then need to manually editted to setup required funtionalities
- matrix require listening to 2 port 8080 for communication and 8448 federation and discovery
- matrix reverse proxy proxy_pass can not be under too many chain it have to be at the first tls reverse proxy or its certificate will be invalid
- When OIDC obtion enable matrix will not start when it failed to contact to the keycloak oidc endpoint
- which cause a problem for an all contained system
- matrix cant start when it cant contact keycloak over nginx
- nginx failed to run when matrix container hostname not found
- solution: separated in to 2 independ proxy and set dns discovery
  
- to use hookshot to create a webhook for grafana notification a custom module have to be registered with matrix
- the configuareion steps are very confusing and documentation is un clear https://matrix-org.github.io/matrix-hookshot/latest/setup/sample-configuration.html
- example: ample Configuration provide serviceBots option but connections is correct option for an alert bot
- the alert bot are failed to respond to command due to E2E encription. to fix it the option, which not mentioned, have to be on
```
...
```
- the message over to the webhook is very verbose and messy and need to be cleanup at the endpoint above

### overrall
- all service can operate cohearativly with each other
- but the communication are happen with restapt call which can be dificult for normal user
- improvement: a custom broker that can communicate with all service and act as bridge between all service.
